...
public class JwtService : IJwtService
{
    ...
    public (JwtResult Status, string? Token) GenerateToken(List<Claim> claims, DateTime? expire)
    {
        try
        {
            ...
            return (JwtResult.Success, tokenHandler.WriteToken(tokenHandler.CreateToken(tokenDescriptor)));
        }
        catch (Exception ex)
        {
            logger.LogError(ex.Message);

            return (JwtResult.Failed(), null);
        }
    }
}
